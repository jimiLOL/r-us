<template>
  <div class="flex flex-col gap-4">
    <h1 :class="['text-bold', $device.isMobile?'text-2xl text-center':'text-4xl']">Контактная информация</h1>
        ...
        >
          <GMapMarker
            ...
            :position="{ lat: location.lat, lng: location.lng }"
             :options="{icon: location === currentLocation ? pins.selected : pins.notSelected, zoom: location === currentLocation ? 5 : 12}"
            @click="currentLocation = location"
          />
        </GMap>
      </div>
      <div :class="['flex', 'flex-col', $device.isMobile?'items-center text-center mb-9':'', 'gap-4']">
        <div
