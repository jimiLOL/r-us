...
const shopApi = {
  ...
  getCategory: async (data: any) => {
    ...
    return await fetch(`${url}/get_category?category_en=${data.category_en}&direction=${data.direction}&offset=${data.offset}&limit=${data?.limit ? data.limit:10}`).then((res) => {
      if (res.ok) {
        return res.json();
      }
    });
  ...
  getPost: async (data: any) => {
    return await fetch(`${url}/get_post?title_en=${data.title_en}`).then((res) => {
      if (res.ok) {
        return res.json();
      }
    }).catch(e=> {
