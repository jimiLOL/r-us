  ...
  >
    ...
    <div v-if="!callFrom && !statusService">
      <button
        v-if="!buttonActive"
        class="
  bg-gradient-to-t from-bt-2 to-bt-1
          text-theme-8
          font-semibold
          py-4
          w-full
    ...
    <div v-if="callFrom && !statusService" class="flex flex-col gap-4">
      ...
      <input
        ...
        v-model="phone"
      /><label
        v-if="phone.length >= 12 && !validation"
        class="text-theme-7"
        for="phone"
      ...
      <button
        :class="[
          ...
          validation
            ? 'bg-gradient-to-t from-bt-2 to-bt-1 hover:bg-theme-10'
            : 'bg-gradient-to-t from-gray-700 to-gray-500 cursor-not-allowed',
        ]"
        @click="sendData"
      >
        <LoadingIcon
    ...
    <div v-if="statusService" class="inline-flex flex-col gap-2">
      ...
      <i
        >Номер вашей заявки: <span class="font-bold">{{ numberBind }}</span></i
      >
